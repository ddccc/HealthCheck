...
import com.Common;
import cc.CCLogin;
import cc.KCallPerson;

public class PeriodLoginCC extends PeriodLogin0 {

  public PeriodLoginCC(String histVecName, String htName, String sessionId) {
    super(histVecName, htName, sessionId);
    label = "Period Login CC";
  }

  public void valueUnbound(HttpSessionBindingEvent event) {
    KCallPerson.removeKCallPerson(sessionId);
    CCLogin.removeCC(sessionId);
    end = new Date();
    ...
    // remove clutter
    Vector chatRecords = (Vector) unboundHashtable.get("chatRecords");
    if ( null != chatRecords && chatRecords.size() <= 0) 
      unboundHashtable.remove("chatRecords");
