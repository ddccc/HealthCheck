// (C) OntoOO Inc Dennis de Champeaux 2003 2012
package patient;
...
public class Diag1SearchLTD  extends HttpServlet {
  private static final boolean LOGGING = DV.LOGGINGdiag1;
	...
	System.out.println("*** Diag1: Enter");
      ...
      session.putValue("searchMode", "searchLTD"); // used in Diag1c
	...
	new DiagStackEntry(locations, systems, 
      ...
      int lng = lifeThreateningDiseaseMembers.size();
      HashSet D2 = new HashSet();
      for (int i=0; i<lng; i++) {
	  String diseaseName = (String) lifeThreateningDiseaseMembers.elementAt(i);
	  if ( !D1.contains(diseaseName) ) continue; // gender check
	  D2.add(diseaseName);
      }
      dse.setD2(D2);

      Diag1a.call("Diag1SearchLTD", 0, session, userInput, null, null, res);
    }
}
