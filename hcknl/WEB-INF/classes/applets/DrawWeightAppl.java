...
public class DrawWeightAppl extends java.applet.Applet {
    int width, height;
  ...
  public void init() {
    ...
    String targetWeightSt = getParameter("targetWeight");
	...
	if (null != targetWeightSt) 
	  targetWeight = Integer.parseInt(targetWeightSt);
	if ( 1 == numberOfPairs ) {
	  xs = new long[2];
	  ys = new int[2];
	} else {
	...
	if (0 < targetWeight && targetWeight - 1 < minY) 
	  minY = targetWeight - 1;
	if ( 1 == numberOfPairs ) { 
  ...
  public void paint( java.awt.Graphics gc ) {
    ...
    if (0 < targetWeight) {
      int y3 = bottom - 
	(int) (height2 * (targetWeight - minY) / (maxY - minY));
      gc.setColor(Color.red);
      gc.drawLine(left, y3, right, y3);
      String yTarget = ""+targetWeight;
      gc.drawString(yTarget, left-10 -5*yTarget.length(), y3+5);
      gc.drawString("target weight", left+5, y3+10);
    ...
    String yTopmin = "" +(int)(minY + (maxY-minY)*0.75);
    gc.drawString(yTopmin,
                  left-10 -5*yTopmin.length(), 
                  (int)(5+bottom+(top-bottom)*0.75));
    String yTopmiddle = "" + (int)(minY + (maxY-minY)*0.5);
    gc.drawString(yTopmiddle,
                  left-10 -5*yTopmiddle.length(), 
                  (int)(5+bottom+(top-bottom)*0.5));
    String yBotplus = "" +(int)(minY + (maxY-minY)*0.25);
    gc.drawString(yBotplus,
                  left-10 -5*yBotplus.length(), 
                  (int)(5+bottom+(top-bottom)*0.25));
    String yBot = ""+minY;
