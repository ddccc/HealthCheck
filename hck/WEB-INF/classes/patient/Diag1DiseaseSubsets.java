...
public class Diag1DiseaseSubsets extends HttpServlet {
  private static final boolean LOGGING = DV.LOGGINGdiag1;
	...
	System.out.println("*** Diag1: Enter");
      ...
      session.putValue("searchMode", "diseaseSubsets"); // used in Diag1c
	...
	new DiagStackEntry(locations, systems, 
      ...
      String[] diseaseSubsetArr = req.getParameterValues("diseasesubset");
      int lng2 = diseaseSubsetArr.length;
      for (int j = 0; j<lng2; j++) {
	  String diseaseSetName = diseaseSubsetArr[j];
	  DiseaseSet diseaseSet = Common.getDiseaseSet(diseaseSetName);
	  Vector diseaseSetMembers = diseaseSet.getDiseaseMembers();
	  int lng = diseaseSetMembers.size();
	  for (int i=0; i<lng; i++) {
	      String diseaseName = (String) diseaseSetMembers.elementAt(i);
      ...
      Diag1a.call("Diag1DiseaseSubsets", 0, session, userInput, null, null, res);
    }
}
