...
public class Diag1Subsystem extends HttpServlet {
  private static final boolean LOGGING = DV.LOGGINGdiag1;
	...
	System.out.println("*** Diag1: Enter");
      ...
      session.putValue("searchMode", "subsystem"); // used in Diag1c
      ...
      String system = req.getParameter("system"); // <<<<< the input for this mode
	...
	new DiagStackEntry(locations, systems, 
      ...
      Vector bsDiseases = bs.getAllDiseases();
      int lng = bsDiseases.size();
      HashSet D2 = new HashSet();
      for (int i=0; i<lng; i++) {
	  String diseaseName = (String) bsDiseases.elementAt(i);
	  if ( !D1.contains(diseaseName) ) continue; // gender check
	  D2.add(diseaseName);
      }
      dse.setD2(D2);

      Diag1a.call("Diag1Subsystem", 0, session, userInput, null, null, res);
    }
}
